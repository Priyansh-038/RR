...
import { Copy, Crown, Check, X } from "lucide-react";
      ...
      <div className="h-screen w-full bg-black flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="absolute top-4 left-4 z-10 text-white font-display text-shadow">
          <div className="flex items-center gap-4">
            ...
            {myPlayer && (
              <div className="bg-black/50 p-2 rounded border-2 border-white/20">
                 HP: {gameState.players.find(p => p.id === myPlayer.id)?.health || 0}
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 flex flex-col gap-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <PixelCard title={`Party (${players.length}/5)`} className="h-full">
            <div className="space-y-3">
              {players.map((p) => (
                <div 
                  key={p.id} 
                  className={`flex items-center justify-between p-3 border-2 ${p.isReady ? 'border-green-500/50 bg-green-500/10' : 'border-muted bg-black/20'}`}
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-muted rounded flex items-center justify-center font-display text-xs">
                      {p.role ? p.role[0].toUpperCase() : "?"}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
        ...
        {/* Right Col: Role Selection & Controls */}
        <div className="lg:col-span-2 space-y-6">
          <PixelCard title="Choose Class">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              {ROLES.map((role) => (
                <RoleCard 
                  ...
                  disabled={myPlayer?.isReady} // Lock role when ready
                />
              ))}
            </div>
          </PixelCard>
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-end items-center border-t-4 border-muted/30 pt-6">
            <div className="text-sm text-muted-foreground font-body mr-auto">
              {!myPlayer?.role && "Select a role to ready up!"}
              {myPlayer?.role && !myPlayer.isReady && "Ready up when you are set!"}
              {myPlayer?.isReady && "Waiting for party..."}
            </div>
            ...
            {isHost && (
              <PixelButton 
                variant="destructive"
                disabled={!players.every(p => p.isReady) || players.length < 1}
                onClick={startGame}
                className="w-full md:w-auto"
              >
                START RAID
